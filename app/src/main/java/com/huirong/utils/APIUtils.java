...
public class APIUtils {
	...
	public static HttpResult postForObject(String url, HttpParameter parameters, boolean withLogin) throws MyException //HttpResult
			...
			try{
				...
				JSONObject jsonObject = com.huirong.utils.HttpUtils.getInstance().postStringURL(url, parameters, withLogin); //

				LogUtils.d("HTTP", "APIUtils--服务端返回="+jsonObject.toString());// 查看响应的信息
	...
	public static HttpResult postForObject(String url, HttpParameter parameters, File fileName) throws MyException {
		try {
			JSONObject jsonObject = com.huirong.utils.HttpUtils.getInstance().postStringURL(url, parameters, fileName); // httpURLConnection
			LogUtils.d("HTTP", jsonObject.toString());// 查看响应的信息
			// 调用本类方法，返回读取的信息，封装在HttpResult中返回给调用方法(登录/注册/验证码)
			return toHttpResult(jsonObject); // 将JSONObject对象-->HttpResult
		} catch (MyException e) {
            throw new MyException(e.getMessage());
	...
	public static HttpResult getForObject(String url) throws MyException {
		...
		try {
			com.huirong.utils.HttpUtils.getInstance();
			JSONObject jsonObject = com.huirong.utils.HttpUtils.getInstance().getByURL(url);
			LogUtils.d("HTTP", jsonObject.toString());
