...
public class ContactsMissionActivity extends BaseActivity {
    //back
    ...
    public void getContactApprover() {
        //先判断sp中是否有值
        ConfigUtil config = new ConfigUtil(ContactsMissionActivity.this);
        List<ContactsEmployeeModel> list = config.getMissionContactData();
    ...
    protected void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case POST_SUCCESEE://服务端获取数据
                ...
                config.setMissionContactData(listContactApprover);
                // 根据a-z进行排序源数据
                Collections.sort(listContactApprover, pinyinComparator);

                adapter = new ContactsSelectAdapter(ContactsMissionActivity.this, listContactApprover);
